...
void queue_init(file_queue_t *q, apr_pool_t *pool) {
    ...
    q->paths = apr_pcalloc(pool, q->num_files * sizeof(char *));

    // Allocate each string buffer
    for (int i = 0; i < q->num_files; i++) {
        q->paths[i] = apr_pcalloc(pool, q->max_pathlen * sizeof(char));
    }
...
int queue_push(file_queue_t *q, const char *path) {
    ...
    while (q->size == q->num_files) {
        apr_thread_cond_wait(q->not_full, q->mutex);
    }
    strncpy(q->paths[q->tail], path, q->max_pathlen - 1);
    q->paths[q->tail][q->max_pathlen - 1] = '\0';
    q->tail = (q->tail + 1) % q->num_files;
    q->size++;
...
int queue_pop(file_queue_t *q, char *out_path) {
    ...
    strncpy(out_path, q->paths[q->head], q->max_pathlen);
    q->head = (q->head + 1) % q->num_files;
    q->size--;
