...
class thread_pool {
  public:
                    ...
                    do {
                        ...
                        do {
                            ...
                            while (auto task = tasks_[id].tasks.pop_front()) {
                                ...
                                // the above task can push more work onto the
                                // pool, so we only decrement the in flights
                                // once the task has been executed because now
                                // it's now longer "in flight"
                            ...
                            for (std::size_t j = 1; j < tasks_.size(); ++j) {
                                ...
                                if (auto task = tasks_[index].tasks.steal()) {
                                    ...
                                    in_flight_tasks_.fetch_sub(
                                        1, std::memory_order_release);
                                    // stop stealing once we have invoked a
                                    // stolen task
                                    break;
                                }
                            }
                            // check if there are any unassigned tasks before
                            // rotating to the front and waiting for more work
                        } while (unassigned_tasks_.load(
                                     std::memory_order_acquire) > 0);

                        priority_queue_.rotate_to_front(id);
                        // check if all tasks are completed and release the
                        // barrier (binary semaphore)
     ...
     * @return A std::future<ReturnType> that can be used to retrieve the
     * returned value.
     */
    template <
 ...
 * Example showing how to use thread pool with tasks that return a value.
 * Outputs a PPM image of a mandelbrot.
 */
} // namespace dp
